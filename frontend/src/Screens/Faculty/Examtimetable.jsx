...
const Examtimetable = () => {
  ...
  const addTimetableHandler = () => {
    toast.loading("Adding Exam Timetable");
    const headers = {
    ...
    formData.append("type", "examtimetable");
    formData.append("examtimetable", file);
    axios.post(`${baseApiURL()}/examtimetable/addTimetable`, formData, { headers: headers })
    .then((response) => {
      toast.dismiss();
      if (response && response.data && response.data.success) {
        toast.success(response.data.message);
        // Reset form state or perform other actions upon successful upload
        setAddSelected({ branch: "", semester: "" });
        setFile(null); // Reset file state
        setPreviewUrl(""); // Reset preview URL
      } else {
        toast.error(response.data ? response.data.message : "Failed to add exam timetable");
      }
    })
    .catch((error) => {
      toast.dismiss();
      console.error("Error adding timetable:", error);
      toast.error(error.response ? error.response.data.message : "Failed to add exam timetable");
    });
  ...
  return (
    <div className="w-full mx-auto mt-10 flex justify-center items-start flex-col mb-10">
      <div className="flex justify-between items-center w-full">
        <Heading title={`Upload Exam Timetable`} />
      </div>
      <div className="w-full flex justify-evenly items-center mt-12">
        <div className="w-1/2 flex flex-col justify-center items-center">
          <p className="mb-4 text-xl font-medium">Add Exam Timetable</p>
          <select
            id="branch"
